// This is additional code created by Olivier. It has not been integrated in the final product
...
// public class EdgesMinimumCrossings {
...
//         autoButton.setOnAction(e -> {giveUp(pane);});
...
//             dialogBox("Hint", "Ideally, you should use " + chromaticNumber + " colors for this graph");
//         });
//     }
...
//         double x = radius + rand.nextDouble() * (innerPane.getWidth() - 2 * radius);
//         double y = radius + rand.nextDouble() * (innerPane.getHeight() - 2 * radius);
//         circle.setCenterX(x);
//         circle.setCenterY(y);
...
//         // Bind index text to circle center
...
//         // Enable drag-and-drop functionality for the circle
//         circleInteractions.makeDraggable(circle, 0, 0, innerPane.getWidth(), innerPane.getHeight());

//         // Set up click event to color the circle
//         circle.setOnMouseClicked(e -> circleInteractions.colorCircle(circle));
//     }

//     // Initialize edges
//     List<Line> edges = new ArrayList<>();
//     Map<Line, Pair<Circle, Circle>> edgeMap = new HashMap<>();

//     // Populate edges and edgeMap in drawEdgesFromBox
//     drawEdgesFromBox(circles, innerPane, numEdges, edges, edgeMap);

// // Call applyFruchtermanReingold with the additional parameters
//     applyFruchtermanReingold(circles, innerPane, edges, edgeMap);
...
// private void applyFruchtermanReingold(ArrayList<Circle> circles, Pane innerPane, List<Line> edges, Map<Line, Pair<Circle, Circle>> edgeMap) {
//     double width = innerPane.getWidth();
//     double height = innerPane.getHeight();
//     double area = width * height;
//     double k = Math.sqrt(area / circles.size())*edges.size()/100;
//     double t = width / 10; // Initial temperature
//     double coolingFactor = 0.95;
//     int iterations = 50;

//     for (int iter = 0; iter < iterations; iter++) {
//         // Reset displacement for each circle
//         Map<Circle, double[]> displacements = new HashMap<>();
//         for (Circle v : circles) {
//             displacements.put(v, new double[]{0, 0});
//         }

//         // Calculate repulsive forces
//         for (int i = 0; i < circles.size(); i++) {
//             Circle v = circles.get(i);
//             for (int j = 0; j < circles.size(); j++) {
//                 if (i != j) {
//                     Circle u = circles.get(j);
//                     double dx = v.getCenterX() - u.getCenterX();
//                     double dy = v.getCenterY() - u.getCenterY();
//                     double distance = Math.sqrt(dx * dx + dy * dy);
//                     if (distance > 0) {
//                         double repulsiveForce = (k * k) / distance;
//                         displacements.get(v)[0] += (dx / distance) * repulsiveForce;
//                         displacements.get(v)[1] += (dy / distance) * repulsiveForce;
//                     }
//                 }
//             }
//         }
...
//         for (Line edge : edges) {
//             Pair<Circle, Circle> pair = edgeMap.get(edge);
//             Circle v = pair.getKey();
//             Circle u = pair.getValue();
...
//                 displacements.get(v)[0] -= (dx / distance) * attractiveForce;
//                 displacements.get(v)[1] -= (dy / distance) * attractiveForce;
//                 displacements.get(u)[0] += (dx / distance) * attractiveForce;
//                 displacements.get(u)[1] += (dy / distance) * attractiveForce;
//             }
//         }

//         // Limit maximum displacement and apply positions
//         for (Circle v : circles) {
//             double[] disp = displacements.get(v);
//             double dispLength = Math.sqrt(disp[0] * disp[0] + disp[1] * disp[1]);
//             if (dispLength > 0) {
//                 double limitedDisp = Math.min(dispLength, t);
//                 double newX = v.getCenterX() + (disp[0] / dispLength) * limitedDisp;
//                 double newY = v.getCenterY() + (disp[1] / dispLength) * limitedDisp;
        
//                 // Reverse direction if displacement exceeds the frame boundaries
//                 if (newX < v.getRadius() || newX > width - v.getRadius()) {
//                     disp[0] = -disp[0]; // Reverse X direction
//                     newX = v.getCenterX() + (disp[0] / dispLength) * limitedDisp;
//                 }
        
//                 if (newY < v.getRadius() || newY > height - v.getRadius()) {
//                     disp[1] = -disp[1]; // Reverse Y direction
//                     newY = v.getCenterY() + (disp[1] / dispLength) * limitedDisp;
//                 }
        
//                 // Constrain new positions to frame boundaries
//                 newX = Math.min(width - v.getRadius(), Math.max(v.getRadius(), newX));
//                 newY = Math.min(height - v.getRadius(), Math.max(v.getRadius(), newY));
...
//      * @param numEdges
//      */
//     private void drawEdgesFromBox(ArrayList<Circle> circles, Pane innerPane, int numEdges, List<Line> edges, Map<Line, Pair<Circle, Circle>> edgeMap) {
//         int n = circles.size();
//         Random rand = new Random();
...
//             line.startXProperty().bind(circles.get(u).centerXProperty());
//             line.startYProperty().bind(circles.get(u).centerYProperty());
//             line.endXProperty().bind(circles.get(v).centerXProperty());
//             line.endYProperty().bind(circles.get(v).centerYProperty());
    
//             innerPane.getChildren().add(line);
//             edges.add(line);
//             edgeMap.put(line, new Pair<>(circles.get(u), circles.get(v)));
//             edgeSet.add(u + "-" + v);
//         }
...
//         ChromaticNumber chromaticNumberMethod = new ChromaticNumber(edgeArray);
//         chromaticNumberMethod.calculateChromaticNumber();
//         chromaticNumber = chromaticNumberMethod.getChromaticNumber();
...
//     /**
//      * Set limit to 50 circles, no graph is drawn as it would be difficult for the user to distinguish vertices and edges
//      * If the graph has been created randomly, we will not calculate the chromatic because, at that point, edges have not been created yet.
//      */
//     private void setLimiter() {
//         if (chromaticNumber == 0) {
//             dialogBox("Number of vertices is limited", "We do not display graphs which have more than 50 vertices.\nSince vertices and edges are created randomly,\nwe do not know how they should be beforehand.\nHence, we cannot provide you with a chromatic number.");
//         } else {
//             dialogBox("Number of vertices is limited", "We do not display graphs which have more than 50 vertices.\nHowever, the chromatic number of this graph is " + chromaticNumber);
//         }
//     }
...
//                     //note for nikos: use autoButton.getUserData() to get circles. 
