...
public class modeTwo {
    ...
    public void start(Stage stage) {
        stage.setTitle("Random order");
        Pane pane = new Pane();

        pane.setBackground(new Background(new BackgroundFill(Color.OLIVE, null, null)));// background color
        ...
        newGameViaBox(pane, colorPicker, innerPane);

        newGameViaTxt(pane, colorPicker, stage, innerPane);

        circleInteractions = new CircleInteractions(colorPicker);

        autoButton = new Button("AUTO");
        autoButton.setLayoutX(1350);
        ...
        Button undoButton = createUndoButton(pane);

        SwitchButton switchColor = createSwitchButtonColor(pane);

        SwitchButton switchMove = createSwitchButtonMove(pane);
    ...
    /*
     * randomize the order in which circles are colored and make sure the order is
     * maintained
     */
    private void randomOrder() {
        ...
        if (!vertices.isEmpty()) {
            Circle firstCircle = vertices.get(0);
            firstCircle.setStroke(Color.RED);
            firstCircle.setStrokeWidth(2);
        }
        ...
        for (Circle circle : vertices) {
            circle.setOnMouseClicked(e -> {
                ...
                if (clicked == vertices.get(currentIndex)) {
                    circleInteractions.colorCircle(circle);
                    clicked.setStroke(Color.WHITE);
                    clicked.setStrokeWidth(1);
                    currentIndex++;
                    if (currentIndex < vertices.size()) {
                        Circle nextCircle = vertices.get(currentIndex);
                        nextCircle.setStroke(Color.RED);
                        nextCircle.setStrokeWidth(2);
                    }
                } else {
                    dialogBox("Error", "You have to color the circle indicated. Try again");
                }
    ...
    private void newGameViaTxt(Pane pane, ColorPicker colorPicker, Stage stage, Pane innerPane) {
        ...
        button.setOnAction(e -> loadTextFile(stage, colorPicker, innerPane));
    ...
    private Button createUndoButton(Pane pane) {
        ...
        button.setLayoutY(10);
        pane.getChildren().add(button);
        button.setOnAction(e -> dialogBox("Undo Coloring Restriction", "Cannot undo coloring in this mode!"));
        return button;
    }
